...
from django.utils.timezones.templatetags import timezone_filters
...
#------------------------------------------------------------------------------
#@login_required
def window(request, id, **kwargs):
    """A basic chat client window."""

    ThisRoom = get_object_or_404(Room, id=id)
    u_user = User.objects.filter(username='Anonymous')[0]
    #if kwargs.get('message', None):
    #    print 'Sem mensagem'
    #else:
    #    Message.objects.create(u_user, ThisRoom, request.GET.get('message'))
    #if request.method == 'POST' and 'message' in request.POST:
    #    query = Message()
    #    query.room = ThisRoom
    #    query.user = User.objects.filter(username='Anonymous')[0]
    #    query.text = request.POST.get('message')
    #    query.save()
...
class Ajax(object):
    ...
    def __call__(self, request, id):
        ...
        if self.request.method == "POST":
            ...
            if action == 'postmsg':
                ...
                if len(msg_text.strip()) > 0: # Ignore empty strings.
                     Message.objects.create_message(User.objects.filter(username='Anonymous')[0], self.ThisRoom, escape(msg_text))
        ...
        # new messages. 
        NewMessages = Message.objects.filter(room=self.ThisRoom,unix_timestamp__gt=self.request_time) #self.ThisRoom.message_set.all() #.filter(unix_timestamp__gt=self.request_time)
        if NewMessages:
            StatusCode = 1
        ...
        #print 'Vai devolver as mensagens:', l
